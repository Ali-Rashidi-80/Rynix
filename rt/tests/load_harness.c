/* Load harness: repeated fiber TCP echo RPS floor (bake-off baseline). */
...
int main(void) {
  ...
  for (int i = 0; i < iters; i++) {
    char msg[32];
    snprintf(msg, sizeof(msg), "L%d", i);
    if (one_echo(40100 + (i % 16), msg) != 0) {
      fprintf(stderr, "load_harness echo %d failed\n", i);
      return 1;
    }
  }
  ...
  printf("load_harness ok  iters=%d  rps=%.1f  (see docs/bakeoff.md)\n", iters, rps);
  if (rps < 1.0) {
    fprintf(stderr, "RPS floor failed: %.1f\n", rps);
    return 1;
  }
  return 0;
}
