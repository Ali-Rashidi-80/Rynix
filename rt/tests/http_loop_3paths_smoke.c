/* Triple-path HTTP serve-loop: mix GETs on /a, /b, and /c until max_reqs. */
...
static void server_fiber(void *arg) {
  (void)arg;
  if (rynix_rt_http_serve_loop_3paths_json_i64(40129, "/a", 11, "/b", 22, "/c", 33, 0) != -1) {
    fprintf(stderr, "expected max_reqs<=0 → -1\n");
    return;
  }
  g_serve = rynix_rt_http_serve_loop_3paths_json_i64(40129, "/a", 11, "/b", 22, "/c", 33, 3);
}

static void client_fiber(void *arg) {
  (void)arg;
  const char *paths[3] = {"/a", "/b", "/c"};
  const int64_t want[3] = {11, 22, 33};
  for (int i = 0; i < 3; i++) {
    int64_t got = -1;
    for (int attempt = 0; attempt < 256; attempt++) {
      rynix_rt_yield();
      got = rynix_rt_http_get_json_i64("127.0.0.1", 40129, paths[i], "value");
      if (got >= 0) {
        break;
      }
    }
...
int main(void) {
  ...
  if (!g_ok) {
    fprintf(stderr,
            "http_loop_3paths failed serve=%lld got=%lld,%lld,%lld\n",
            (long long)g_serve, (long long)g_got[0], (long long)g_got[1],
            (long long)g_got[2]);
    return 1;
  }
  puts("http_loop_3paths ok");
  return 0;
}
