# Phase 15 — Run emitted `.wasm` + Skills pack (still no WASI)

**Status:** **Phase 15 Waves A–B complete** (2026-08-25)  
...
3. Agent Skills stay **docs packs** (`.agents/skills/…`), never End `feature`/
   `skill` language keywords ([ADR-0009](adr/0009-agent-contracts-toolchain.md)).
4. Still **no** WASI libc, no host `rt/` link, no browser UI, no llama.cpp /
   Raft theater (those stay later only with real FFI gates).
...
| Skills-as-docs agent pack | **Wave B** — refresh `.agents/skills/rynix/SKILL.md` |
...
`0 (docs locks) → A (Node runs emit-wasm main) → B (Skills pack honesty gate)`
...
| P15-L5 | Textual LLVM + clang link path unchanged ([PHASE14.md](PHASE14.md)); Wave A only adds a run gate. |
| P15-L6 | Wave B Skills pack documents `emit-wasm` + `deps --attest` and states it is **not** a language keyword. Gate: `agent_skill_mentions_emit_wasm_and_attest`. |
...
| B | `agent_skill_mentions_emit_wasm_and_attest` | Skills-as-docs pack stays honest |

## Refuse

Full WASI, `rt/` in wasm, UI/canvas, GGUF reimplementation, Raft Stable rows,
Sigstore Rekor theater, Suite5 micro-opt theater, End `feature`/`skill` keywords.
