...
**Status:** **Phase 14 Wave A complete** (2026-08-25)  
...
3. Sigstore-lite package attest and deeper HTTP/fiber smokes stay **out of Wave A**
   (listed as later candidates below).
...
| Local package lock/sign (Sigstore-lite) | Later — sparse index already ships ([ADR-0010](adr/0010-local-package-index.md)) |
| Deeper I/O (HTTP ≥3 paths / fiber smokes) | Later — 2-path loop already green in Phase 13 |

## Order

`0 (docs locks) → A (emit-wasm + clang link smoke)`
