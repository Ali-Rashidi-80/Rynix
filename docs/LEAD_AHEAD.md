...
| **CDN / PubGrub registry** | At *scale* CDN matters ([crates.io 2024 CDN](https://blog.rust-lang.org/2024/03/11/crates-io-download-changes/)); young langs start **path/git/local index** (Mochi/Cargo prior art). Fake PubGrub = End theater. | **Refuse P12 CDN** (ADR-0010) | **Local sparse index shipped** (`deps_resolves_sparse_local_index`); Sigstore + CDN when download volume hurts |
