...
| MCP | Ready | **18** tools; path-first `check`/`context`/`security`/`apply_fix`/`format`/`explain_alloc`/`compile`/`ast_query`/`graph`/`impact`/`precheck` (fail-closed) | stdio JSON-RPC only |
...
| Editor (LSP) | Ready | `lsp-serve` diag/hover/def/**completion**/**rename**/**references**/**workspace/symbol**; VS Code ext + **CodeLens** | No studio/canvas ([ADR-0007](docs/adr/0007-deferred-ui-frameworks.md)) |
